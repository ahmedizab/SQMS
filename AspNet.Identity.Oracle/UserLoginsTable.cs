...
namespace AspNet.Identity.Oracle
{
    ...
    public class UserLoginsTable
    {
        ...
        {
            const string commandText = @"DELETE FROM ASPNETUSERLOGINS WHERE USERID = :USERID AND LOGINPROVIDER = :LOGINPROVIDER AND PROVIDERKEY = :PROVIDERKEY";
        ...
        {
            const string commandText = @"DELETE FROM ASPNETUSERLOGINS WHERE USERID = :USERID";
            var parameters = new List<OracleParameter>
            {
        ...
        {
            const string commandText = @"INSERT INTO ASPNETUSERLOGINS (LOGINPROVIDER, PROVIDERKEY, USERID) VALUES (:LOGINPROVIDER, :PROVIDERKEY, :USERID)";
            var parameters = new List<OracleParameter>
            {
                new OracleParameter{ ParameterName = "USERID", Value = user.Id, OracleDbType = OracleDbType.Varchar2 },
                new OracleParameter{ ParameterName = "LOGINPROVIDER", Value = login.LoginProvider, OracleDbType = OracleDbType.Varchar2 },
        ...
        {
            const string commandText = @"SELECT * FROM ASPNETUSERLOGINS WHERE USERID = :USERID";
            var parameters = new List<OracleParameter>
            {
                new OracleParameter{ ParameterName = "USERID", Value = userId, OracleDbType = OracleDbType.Varchar2 },
            };
