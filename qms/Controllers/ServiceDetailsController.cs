    ...
    public class ServiceDetailsController : Controller
    {
        ...
        public JsonResult GetCustomerInformation(long token_id, string contact_no)
        {
            try
            {
                ...
                tblTokenQueue tblTokenQueue = db.tblTokenQueues.Where(a => a.token_id == token_id).FirstOrDefault();
                tblCustomer customerDetails = db.tblCustomers.Where(a => a.contact_no == contact_no).FirstOrDefault();
                ...
                if (customerDetails != null)
                {
                    ...
                    List<tblServiceDetail> previousHistoryList = db.tblServiceDetails.Where(x => x.customer_id == customerDetails.customer_id).ToList();
