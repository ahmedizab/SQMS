...
(
  SMS_LOG_ID  NUMBER 											NOT NULL,
  MSISDN      VARCHAR2(20)      								NOT NULL,
  MESSAGE     VARCHAR2(250)     								NOT NULL,
  SEND_TIME   DATE              			DEFAULT SYSDATE		NOT NULL,
  IS_SENT     NUMBER                        DEFAULT 0       	NOT NULL
   CONSTRAINT PK_tblSmsLog PRIMARY KEY 
(
  SMS_LOG_ID 
) 
);

CREATE SEQUENCE tblSmsLog_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER tblSmsLog_seq_tr
 BEFORE INSERT ON tblSmsLog FOR EACH ROW
 WHEN (NEW.sms_log_id IS NULL)
BEGIN
 SELECT tblSmsLog_seq.NEXTVAL INTO :NEW.sms_log_id FROM dual;
END;
...
ALTER TABLE AspNetUserRoles ADD FOREIGN KEY (RoleId) REFERENCES AspNetRoles(Id) ON DELETE CASCADE;
ALTER TABLE AspNetUserRoles ADD FOREIGN KEY (UserId) REFERENCES AspNetUsers(Id) ON DELETE CASCADE; 
ALTER TABLE tblBranchUser ADD FOREIGN KEY (user_id) REFERENCES AspNetUsers(Id) ON DELETE CASCADE; 
...
CREATE OR REPLACE PROCEDURE    USP_CustomerType_Edit (
   p_Customer_type_id       NUMBER,
   po_cursor           OUT SYS_REFCURSOR)
IS
BEGIN
        ...
        FROM (  SELECT T.TOKEN_ID,
                 ...
                 WHERE     (   (    T.counter_id = P_COUNTER_ID
                            ...
                            OR (    T.counter_id IS NULL
                                ...
                                AND T.branch_id = P_BRANCH_ID))
                       AND TRUNC (T.service_date) = TRUNC (SYSDATE)
              ORDER BY NVL(CT.PRIORITY,99), T.token_NO ASC) TBL
       WHERE ROWNUM <= 1;
   END IF;
...
INSERT INTO tblCustomerType (customer_type_name) VALUES (N'ICON');
...
-- NEWLY ADDED
ALTER TABLE TBLCUSTOMERTYPE  ADD (PRIORITYS NUMBER DEFAULT 1 NOT NULL);